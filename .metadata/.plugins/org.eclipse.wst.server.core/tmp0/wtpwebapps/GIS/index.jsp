...
<article>
	...
	<script>
		...
		for (int i = 0; i < markerArr.size(); i++) {
			...
			%>
		    	...
		    	content = content + '<input type="hidden" name="xCoord" value="<%= markerArr.get(i).getxCoord() %>">';
		    	content = content + '<input type="hidden" name="yCoord" value="<%= markerArr.get(i).getyCoord() %>">';
	...
	<tbody>
		...
		%>
				...
				<td><a href="detail.do?xCoord=<%= markerArr.get(i).getxCoord() %>&yCoord=<%= markerArr.get(i).getyCoord() %>&address=<%= markerArr.get(i).getAddress() %>&uName=<%= markerArr.get(i).getuName() %>&uGroup=<%= markerArr.get(i).getuGroup() %>"><%= arr.get(i).getAddress() %></a></td>
				</tr>
		<%
			}
		}
