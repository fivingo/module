...
public class ReWrite implements CommandHandler {
	@Override
	public String process(HttpServletRequest req, HttpServletResponse resp)
		...
		int ref = Integer.parseInt(multiReq.getParameter("ref"));
		int step = Integer.parseInt(multiReq.getParameter("step"));
		int position = Integer.parseInt(multiReq.getParameter("position"));
		
		ListDAO listDao = new ListDAO();
		ListDTO listDto = new ListDTO(0, title, userId, null, content, 'N', 0, ref, step, position);
		
		// 게시물 DB 저장
		int result = listDao.reWrite(listDto);
